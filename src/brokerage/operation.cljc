(ns brokerage.operation
  "OperationActor -- one securities-brokerage operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Broker-LLM) is sealed into a single node (:advise); its proposal is
  ALWAYS routed through the Brokerage Governor (:govern) and the
  ...
  One graph run = one securities-brokerage operation (intake -> advise
  ...
  decision to a human operator (the licensed broker-dealer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:trade/execute` ALWAYS reaches this node when the
  governor is clean -- see `brokerage.phase`."
...
(defn build
  ...
  `brokerage.store/Store`).
  opts:
    :advisor      -- a `brokerage.brokerllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (brokerllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Broker-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (brokerllm/-advise advisor store request)]
            {:proposal p :audit [(brokerllm/trace request p)]})))

      ;; Brokerage Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
