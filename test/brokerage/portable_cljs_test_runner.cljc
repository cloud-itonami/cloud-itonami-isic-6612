(ns brokerage.portable-cljs-test-runner
  ...
  `brokerage.store-contract-test`, which exercises the langchain.db
  Datomic-API-compatible store — the kotoba-server / kotobase datom
  seam — under ClojureScript, and `brokerage.corporate-intel-test`'s
  cross-reference into cloud-itonami-isic-8291.
  ...
  -m if combined, hence -Sdeps for the extra path):
    ...
    clojure -Sdeps '{:paths [\"src\" \"test\"]}' \\
      ...
      --output-to target/tests.cjs -c brokerage.portable-cljs-test-runner
  ...
  CommonJS."
  (:require [brokerage.corporate-intel-test]
            [brokerage.facts-test]
            [brokerage.governor-contract-test]
            [brokerage.kernels.gate-test]
            [brokerage.phase-test]
            [brokerage.registry-test]
            [brokerage.store-contract-test]
...
(defn -main []
  (run-tests 'brokerage.facts-test
             'brokerage.registry-test
             'brokerage.phase-test
             'brokerage.kernels.gate-test
             'brokerage.governor-contract-test
             'brokerage.store-contract-test
             'brokerage.corporate-intel-test))
